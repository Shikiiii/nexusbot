...
async def on_ready():
	...
	global role_names
	role_names = {"red", "yellow", "black", "maroon", "darkred", "brown", "firebrick", "crimson", "tomato", "coral", "indianred", "lightcoral", "darksalmon", "salmon", "lightsalmon", "orangered", "darkorange", "orange", "gold", "darkgoldenrod", "goldenrod", "palegoldenrod", "pink", "lightpink", "hotpink", "deeppink", "darkkhaki", "palevioletred", "khaki", "mediumvioletred", "orchid", "olive", "magenta", "violet", "plum", "thistle", "yellow", "purple", "yellowgreen", "darkolivegreen", "olivedrab", "mediumorchid", "lawngreen", "darkorchid", "darkviolet", "darkmagenta", "mediumpurple", "mediumslateblue", "greenyellow", "slateblue"}
	...
	if(message.channel.id == "599640898233565198"):
		...
		if message.content in role_names:
			...
			if hasattr(new_role, "id"):
				...
				print("Great success!")
				embed = discord.Embed(title="You successfully changed your color to **{}**.".format(str(new_role)), description="Enjoy your new color! | Nexus", color=new_role.colour)
				await bot.send_message(message.author, embed=embed)
		else:
			await bot.send_message(message.author, ":x: | You entered an invalid color. Please note that the colors are case sensitive, so spell them as you see them.")
			await bot.delete_message(message)
